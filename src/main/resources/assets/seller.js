...
var url = "/malgudi/seller/fetch?sellerId=" + sellerId;

$.get(url, {sellerId:sellerId}, function(cancelledOrders){
...
$.each(cancelledOrders, function(){
    ...
    if(cancelledOrders[orderIterator].approvalState == null || cancelledOrders[orderIterator].approvalState == "NOT_OPTED_IN"){
     ...
     if(cancelledOrders[orderIterator].approvalState == "OPTED_IN"){
            $('#'+cancelledOrders[orderIterator].orderId).append("<div class='order-text'><div class='rhs-container'><div class='optin-btn-container'>Bid Price: "+cancelledOrders[orderIterator].bidOrderPrice+" <br />Bid SLA: "+cancelledOrders[orderIterator].bidSla+"<br /><br /><input data='"+cancelledOrders[orderIterator].orderId+"' productId='"+cancelledOrders[orderIterator].productId+"' listingLQS='"+cancelledOrders[orderIterator].listingLQS+"' productId='"+cancelledOrders[orderIterator].orderId+"' class='optin-btn' type='submit' value='Opted In'/></div></div></div>");
     }

    if(cancelledOrders[orderIterator].approvalState == "ORDER_PLACED"){
            $('#'+cancelledOrders[orderIterator].orderId).append("<div class='order-text'><div class='rhs-container'><div class='optin-btn-container'>Bid Price: "+cancelledOrders[orderIterator].bidOrderPrice+" <br />Bid SLA: "+cancelledOrders[orderIterator].bidSla+"<br /><br /><input data='"+cancelledOrders[orderIterator].orderId+"' productId='"+cancelledOrders[orderIterator].productId+"' listingLQS='"+cancelledOrders[orderIterator].listingLQS+"' productId='"+cancelledOrders[orderIterator].orderId+"' class='optin-btn' type='submit' value='Order Placed'/></div></div></div>");
     }
